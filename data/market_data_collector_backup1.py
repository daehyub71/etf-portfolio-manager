...
class MarketDataCollector:
    ...
    # ==========================================
    # 654개 전체 ETF 수집을 위한 수정된 get_all_etf_list 메서드
    # ==========================================

    def get_all_etf_list(self) -> List[Dict]:
        """전체 ETF 종목 리스트 조회 (654개 전체 수집 버전)"""
        ...
        try:
            ...
            # 🔧 수정: 안전 ETF 검증 건너뛰기 (모든 필터된 ETF 사용)
            safe_tickers = filtered_tickers  # 모든 필터된 ETF 사용
            ...
            # 🔧 수정: 배치 크기 증가 및 전체 처리 설정
            batch_size = 10  # 배치 크기 증가로 속도 향상
            success_count = 0
            error_count = 0
            
            # 🔧 수정: 전체 ETF 처리 (제한 해제)
            max_etfs = len(safe_tickers)     # 전체 처리
            processing_tickers = safe_tickers # 전체 사용
            
            logger.info(f"처리할 ETF: {len(processing_tickers)}개 (전체 처리 모드)")
            
            # 🔧 수정: 진행률 계산을 위한 총 개수 저장
            total_etfs = len(processing_tickers)
            
            for i in range(0, len(processing_tickers), batch_size):
                batch_tickers = processing_tickers[i:i+batch_size]
                
                # 🔧 수정: 진행률 표시 개선
                progress = ((i + len(batch_tickers)) / total_etfs) * 100
                logger.info(f"ETF 정보 조회: {i+1}-{min(i+batch_size, len(processing_tickers))}/{len(processing_tickers)} ({progress:.1f}%)")
                ...
                # 🔧 수정: 배치마다 진행 상황 로깅 (조기 종료 조건 제거)
                current_progress = ((i + batch_size) / total_etfs) * 100
                logger.info(f"진행 상황: {len(etf_list)}개 수집됨 ({current_progress:.1f}% 완료, 성공률: {success_count}/{success_count+error_count})")
                
                # 🔧 수정: 조기 종료 조건 완전 제거 (전체 처리를 위해)
                # 기존: if len(etf_list) >= 20: break  <- 이 부분 삭제됨
                
                # 배치마다 적절한 지연 (서버 부하 방지)
                if i + batch_size < len(processing_tickers):
                    time.sleep(1.0)  # 1초 지연으로 안전하게
                
                # 🔧 수정: 중간 저장 (메모리 효율성을 위해 100개마다)
                if len(etf_list) % 100 == 0 and len(etf_list) > 0:
                    logger.info(f"🎯 중간 저장: {len(etf_list)}개 ETF 수집 완료")
            
            # 🔧 수정: 최종 결과 로깅 개선
            final_success_rate = (success_count / (success_count + error_count)) * 100 if (success_count + error_count) > 0 else 0
            logger.info(f"✅ ETF 정보 수집 완료: {len(etf_list)}개 (성공: {success_count}, 실패: {error_count}, 성공률: {final_success_rate:.1f}%)")
            
            # 🔧 수정: 최소 개수 조건 상향 (전체 수집 목표)
            min_required_etfs = max(50, total_etfs // 10)  # 최소 50개 또는 전체의 10%
            if len(etf_list) < min_required_etfs:
                logger.warning(f"수집된 ETF가 목표({min_required_etfs}개)보다 부족함 - 기본 리스트로 보완")
    ...
    def get_all_etf_list_with_options(self, max_etfs: int = None, fast_mode: bool = False) -> List[Dict]:
        """옵션이 있는 ETF 리스트 조회"""
        
        if fast_mode:
            # 빠른 모드: 배치 크기 증가, 지연 시간 단축
            original_batch_size = 10
            original_delay = self.api_delay
            
            batch_size = 20
            self.api_delay = 0.1
            
            logger.info("🚀 빠른 모드로 ETF 조회 시작")
        else:
            batch_size = 5
            logger.info("🛡️ 안전 모드로 ETF 조회 시작")
        
        try:
            # 기본 get_all_etf_list 로직 사용하되 옵션 적용
            etf_list = self.get_all_etf_list()
            
            # max_etfs 제한 적용
            if max_etfs and len(etf_list) > max_etfs:
                logger.info(f"결과를 {max_etfs}개로 제한합니다")
    ...
    def demonstrate_full_collection():
        """전체 ETF 수집 데모"""
        
        print("🚀 전체 ETF 수집 데모")
        print("=" * 50)
        
        collector = MarketDataCollector(safe_mode=False)  # 빠른 처리를 위해 안전 모드 해제
        
        print("\n📊 옵션 선택:")
        print("1. 전체 ETF 수집 (654개, 약 10-15분)")
        print("2. 상위 100개만 빠르게 수집 (약 2-3분)")
        print("3. 빠른 모드로 전체 수집 (약 5-8분)")
        ...
        if choice == "1":
            ...
            # 상위 100개만
            print("\n⚡ 상위 100개 ETF 빠른 수집...")
            etf_list = collector.get_all_etf_list_with_options(max_etfs=100)
            
        elif choice == "3":
            # 빠른 모드 전체 수집
            print("\n🚀 빠른 모드로 전체 수집...")
            etf_list = collector.get_all_etf_list_with_options(fast_mode=True)
            
        else:
            print("❌ 잘못된 선택입니다.")
            return
        
        print(f"\n✅ 수집 완료: {len(etf_list)}개 ETF")
        ...
        if etf_list:
            ...
            for i, etf in enumerate(etf_list[:10]):
                aum = etf.get('aum', 0)
                print(f"{i+1:2d}. {etf['name']:20s} ({etf['code']}) - {etf['market_price']:,}원, AUM: {aum:,}억원")
...
if __name__ == "__main__":
    # 즉시 실행 가능한 전체 수집
    MarketDataCollector.demonstrate_full_collection()
